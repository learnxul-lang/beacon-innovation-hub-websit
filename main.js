...
async function renderUpdates(targetId, limit){
  ...
  if (!root) return;
  let posts = await STORE.byType('update');
...
async function splitEvents(){
  const now = new Date();
  const posts = (await STORE.byType('event')).sort((a,b) => new Date(a.eventDate) - new Date(b.eventDate));
  return {
...
async function articleCategories(){
  return [...new Set((await STORE.byType('article')).map(post => post.category || 'Software Engineering'))];
}
...
async function renderArticles(targetId, category='', limit){
  const root = document.getElementById(targetId);
  if (!root) return;
  let posts = await STORE.byType('article');
...
async function renderGallery(targetId){
  const root = document.getElementById(targetId);
  if (!root) return;
  const posts = (await STORE.byType('media')).filter(post => safeMediaSrc(post.image));
