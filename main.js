...
function getStore() {
  if (!window.STORE) {
    throw new Error(
      'Website data could not be loaded.'
    );
  }
...
async function renderUpdates(
  ...
  try {
    let posts = await getStore().byType(
      'update'
    );
...
async function splitEvents() {
  ...
  const posts = (
    await getStore().byType('event')
  ).sort(
    (first, second) =>
      new Date(first.eventDate) -
      new Date(second.eventDate)
  );

  return {
    upcoming: posts.filter(
      post =>
        new Date(post.eventDate) >= now
    ),
    past: posts
      .filter(
        post =>
...
async function renderGallery(targetId) {
  ...
  try {
    ...
    root.innerHTML = `
      <div class="empty">
        Gallery photographs could not be loaded.
      </div>
    `;
  }
}
